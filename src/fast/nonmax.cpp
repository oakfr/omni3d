...
// positive = 1 if positive corner, 0 otherwise
...
xy*  fast_nonmax(const byte* im, int xsize, int ysize, xy* corners, int numcorners, int barrier, int* numnx)
{
	...
	for(i=0; i< numcorners; i++)
	{
		...
		scores[i] = corner_score(im + corners[i].x + corners[i].y * xsize, pointer_dir, barrier, positive);
		corners[i].positive = positive; //  08/30/2006
	}
	...
	for(i=1; i < numcorners-1; i++)
	{
		...
		nonmax_corners[num_nonmax].positive = corners[i].positive; // 08/30/2006
