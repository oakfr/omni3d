...
void ConvBuffer(double *buffer, double *kernel, int rsize, int ksize);
void ConvBufferFast(double *buffer, double *kernel, int rsize, int ksize);
...
/* Allocate memory for a 2D double matrix of size [row,col].  This returns
...
double **AllocMatrix(int rows, int cols, int pool)
{
    ...
    m = (double **) MallocPool(rows * sizeof(double *), pool);
    v = (double *) MallocPool(rows * cols * sizeof(double), pool);
    for (i = 0; i < rows; i++) {
	m[i] = v;
	v += cols;
    }
...
SiftImage CopyImage(SiftImage image, int pool)
{
    ...
    newc = CreateImage(image->rows, image->cols, pool);

    for (r = 0; r < image->rows; r++)
      for (c = 0; c < image->cols; c++)
	newc->pixels[r][c] = image->pixels[r][c];
    return newc;
}


/*----------------------- SiftImage utility routines ----------------------*/
