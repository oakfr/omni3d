...
*/
const double GaussTruncate = 4.0;
...
/* Magnitude of difference-of-Gaussian value at a keypoint must be
   ...
   low-contast regions.
*/
const double PeakThreshInit = 0.08; // 0.03 in Lowe's library
double PeakThresh;
...
*/
const double EdgeEigenRatio = 10.0;
...
*/
const double OriSigma = (UseHistogramOri ? 1.5 : 1.0);
...
/* All local peaks in the orientation histogram are used to generate
   ...
   at each location.
*/
const double OriHistThresh = 0.8;
...
/* Width of Gaussian weighting window for index vector values.  It is
   ...
   worse.
*/
const double IndexSigma = 1.0;  /* Was 1.0 */
...
Keypoint OctaveKeypoints(SiftImage image, SiftImage *pnextImage, double octSize,
			 Keypoint keys);
Keypoint FindMaxMin(SiftImage *dogs, SiftImage *blur, double octSize, Keypoint keys);
int LocalMaxMin(double val, SiftImage dog, int row, int col);
int NotOnEdge(SiftImage dog, int r, int c);
Keypoint InterpKeyPoint(SiftImage *dogs, int s, int r, int c, SiftImage grad,
	SiftImage ori, SiftImage map, double octSize, Keypoint keys, int movesRemain);
double FitQuadratic(double offset[3], SiftImage *dogs, int s, int r, int c);
Keypoint AssignOriHist(SiftImage grad, SiftImage ori, double octSize, double octScale,
		       double octRow, double octCol, Keypoint keys);
void SmoothHistogram(double *hist, int bins);
double InterpPeak(double a, double b, double c);
Keypoint AssignOriAvg(SiftImage grad, SiftImage ori, double octSize, double octScale,
			double octRow, double octCol, Keypoint keys);
Keypoint MakeKeypoint(SiftImage grad, SiftImage ori, double octSize, double octScale,
		      double octRow, double octCol, double angle, Keypoint keys);
void MakeKeypointSample(Keypoint key, SiftImage grad, SiftImage ori, double scale,
			double row, double col);
void NormalizeVec(double *vec, int len);
void KeySampleVec(double *vec, Keypoint key, SiftImage grad, SiftImage ori,
		  double scale, double row, double col);
void KeySample(double index[IndexSize][IndexSize][OriSize], Keypoint key,
	       SiftImage grad, SiftImage ori, double scale, double row, double col);
void AddSample(double index[IndexSize][IndexSize][OriSize], Keypoint k,
	       SiftImage grad, SiftImage orim, int r, int c, double rpos, double cpos,
	       double rx, double cx);
void PlaceInIndex(double index[IndexSize][IndexSize][OriSize],
		  double mag, double ori, double rx, double cx);
...
{
    int minsize;
    double curSigma, sigma, octSize = 1.0;
...
{
    int i;
    double sigmaRatio, prevSigma, increase;
    //SiftImage blur[Scales+3], dogs[Scales+2];
	SiftImage *blur = (SiftImage*)malloc((Scales+3)*sizeof(SiftImage));
	SiftImage *dogs = (SiftImage*)malloc((Scales+2)*sizeof(SiftImage));
    ...
    */
    sigmaRatio = pow(2.0, 1.0 / (double) Scales);
    ...
    */
    ...
    /* SiftImage blur[Scales] has twice the blur of starting image for
...
int NotOnEdge(SiftImage dog, int r, int c)
{
    double H00, H11, H01, det, trace, inc;
    double **d = dog->pixels;
...
{
    int newr = r, newc = c;
    double offset[3], octScale, peakval;
    ...
    */
	...
	return AssignOriHist(grad, ori, octSize,
			    InitSigma * pow(2.0, s / (double) Scales),
			    (double) r, (double) c, keys);
    }
    ...
    */
    octScale = InitSigma * pow(2.0, (s + offset[0]) / (double) Scales);
...
/* Assign an orientation to this keypoint.  This is done by creating a
     ...
     The results are in the range of -PI_SIFT to PI_SIFT.
*/
Keypoint AssignOriHist(SiftImage grad, SiftImage ori, double octSize, double octScale,
		       double octRow, double octCol, Keypoint keys)
{
   int i, r, c, row, col, rows, cols, radius, bin, prev, next;
   double hist[OriBins], distsq, gval, weight, angle, sigma, interp,
         maxval = 0.0;
      ...
      for (c = col - radius; c <= col + radius; c++)
         ...
         if (r >= 0 && c >= 0 && r < rows - 2 && c < cols - 2) {
            ...
            if (gval > 0.0  &&  distsq < radius * radius + 0.5) {
               ...
               /* Ori is in range of -PI_SIFT to PI_SIFT. */
               angle = ori->pixels[r][c];
               bin = (int) (OriBins * (angle + PI_SIFT + 0.001) / (2.0 * PI_SIFT));
               assert(bin >= 0 && bin <= OriBins);
               bin = MIN_SIFT(bin, OriBins - 1);
   ...
   */
   for (i = 0; i < OriBins; i++) {
	 ...
	 hist[i] >= OriHistThresh * maxval) {
	  ...
	  Set angle in range -PI_SIFT to PI_SIFT.
       */
       interp = InterpPeak(hist[prev], hist[i], hist[next]);
       angle = 2.0 * PI_SIFT * (i + 0.5 + interp) / OriBins - PI_SIFT;
       assert(angle >= -PI_SIFT  &&  angle <= PI_SIFT);
...
/* Alternate approach to return an orientation for a keypoint, as
   ...
   in the region.  The result is in the range of -PI_SIFT to PI_SIFT.  This was
		      ...
		      double octRow, double octCol, Keypoint keys)
{
   ...
   double gval, sigma, distsq, weight, angle, xvec = 0.0, yvec = 0.0;
      ...
      for (c = icol - radius; c <= icol + radius; c++)
         if (r >= 0 && c >= 0 && r < rows && c < cols) {
            ...
            if (distsq <= radius * radius) {
               weight = exp(- distsq / (2.0 * sigma * sigma));
               /* Angle is in range of -PI_SIFT to PI_SIFT. */
               angle = ori->pixels[r][c];
	       xvec += weight * gval * cos(angle);
	       yvec += weight * gval * sin(angle);
            }
         }
   /* atan2 returns angle in range [-PI_SIFT,PI_SIFT]. */
			...
			double row, double col)
{
   ...
   */
   ...
   /* Convert double vector to integer. Assume largest value in normalized
   ...
   */
   ...
   for (i = 0; i < VecLength; i++) {
     ...
     assert(intval >= 0);
     key->ivec[i] = (unsigned char) MIN_SIFT(255, intval);
   }
}
...
{
   int i, j, k, v;
   double index[IndexSize][IndexSize][OriSize];
	       ...
	       SiftImage grad, SiftImage ori, double scale, double row, double col)
{
   ...
   double spacing, radius, sine, cosine, rpos, cpos, rx, cx;
   ...
   for (i = -iradius; i <= iradius; i++)
     for (j = -iradius; j <= iradius; j++) {
       ...
       */
       ...
       if (rx > -1.0 && rx < (double) IndexSize  &&
	   cx > -1.0 && cx < (double) IndexSize)
	       ...
	       double rx, double cx)
{
    ...
    */
    ...
    /* Put orientation in range [0, 2*PI_SIFT].  If sign of gradient is to
       be ignored, then put in range [0, PI_SIFT].
    */
    if (IgnoreGradSign) {
       while (ori > PI_SIFT)
          ori -= PI_SIFT;
...
void PlaceInIndex(double index[IndexSize][IndexSize][OriSize],
		  double mag, double ori, double rx, double cx)
{
   int r, c, or, ri, ci, oi, rindex, cindex, oindex;
   double oval, rfrac, cfrac, ofrac, rweight, cweight, oweight;
   double *ivec;
   
   oval = OriSize * ori / (IgnoreGradSign ? PI_SIFT : 2*PI_SIFT);
      ...
      if (rindex >=0 && rindex < IndexSize) {
         ...
         for (c = 0; c < 2; c++) {
            ...
            if (cindex >=0 && cindex < IndexSize) {
               ...
               for (or = 0; or < 2; or++) {
                  oindex = oi + or;
                  if (oindex >= OriSize)  /* Orientation wraps around at PI_SIFT. */
                     oindex = 0;
                  oweight = cweight * ((or == 0) ? 1.0 - ofrac : ofrac);
